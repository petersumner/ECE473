...
grader.addHiddenPart('1-hidden', test1, 3, description="Hidden test for CounterexampleMDP. Ensure that V[startState] is greater after noise is added.")
...
grader.addBasicPart('2a-basic', test2a, 5, description="Basic test for succAndProbReward() that covers several edge cases.")
...
def test2aHidden():
    ...
    alg.solve(mdp, .0001)
grader.addHiddenPart('2a-hidden', test2aHidden, 5, description="Hidden test for ValueIteration. Run ValueIteration on BlackjackMDP, then test if V[startState] is correct.")
...
grader.addBasicPart('3a-basic', test3a, 5, maxSeconds=10, description="Basic test for incorporateFeedback() using NumberLineMDP.")
...
grader.addHiddenPart('3a-hidden', test3aHidden, 3, maxSeconds=3, description="Hidden test for incorporateFeedback(). Run QLearningAlgorithm on smallMDP, then ensure that getQ returns reasonable value.")
...
grader.addBasicPart('3c-basic', test3c, 5, maxSeconds=10, description="Basic test for blackjackFeatureExtractor.  Runs QLearningAlgorithm using blackjackFeatureExtractor, then checks to see that Q-values are correct.")
...
grader.addBasicPart('3c-helper', run3cHelper, 0, maxSeconds=60, description="Helper function to compare rewards when simulating RL over two different MDPs in question 3c.")
